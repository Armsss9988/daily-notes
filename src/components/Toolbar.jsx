...
export default function Toolbar({ editor, variant = 'inline' }) {
  ...
  return (
    <div className="flex items-center gap-1 px-3 py-1.5 bg-gray-50 dark:bg-gray-800/50 flex-wrap shrink-0">
      <button className={btnClass(editor.isActive('bold'))} onClick={() => editor.chain().focus().toggleBold().run()} title="Bold (Ctrl+B)">
        <strong>B</strong>
      </button>
      <button className={btnClass(editor.isActive('italic'))} onClick={() => editor.chain().focus().toggleItalic().run()} title="Italic (Ctrl+I)">
        <em>I</em>
      </button>
      <button className={btnClass(editor.isActive('underline'))} onClick={() => editor.chain().focus().toggleUnderline().run()} title="Underline (Ctrl+U)">
        <u>U</u>
      </button>
      ...
      <div className="relative">
        <button className={btnClass(false)} onClick={() => { setShowBg(!showBg); setShowColor(false); }} title="Highlight">
          <span style={{ color: '#fbbf24' }}>HL</span>
        </button>
        {showBg && (
          <div className="absolute top-full left-0 mt-1 p-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-10 flex flex-wrap gap-1 w-40">
            {['#fbbf24', '#f87171', '#34d399', '#60a5fa', '#fef08a', '#fecaca', '#d9f99d', '#bfdbfe'].map(c => (
              <button
                ...
                className="w-6 h-6 rounded border border-gray-300 dark:border-gray-600"
                style={{ backgroundColor: c }}
                onClick={() => { editor.chain().focus().setHighlight({ color: c }).run(); setShowBg(false); }}
              />
            ))}
          </div>
        )}
      ...
      <select
        className="text-xs px-1 py-1 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300"
        value={currentFontSize}
        onChange={(e) => {
          const size = parseInt(e.target.value);
          editor.chain().focus().setFontSize(size).run();
